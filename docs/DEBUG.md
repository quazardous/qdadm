...
ref per element, headed by the layout, page component, entity permissions (with
why each action is allowed or not) and the page's state (rows, filters, dirty fields),
with qdadm zones and their blocks as lines), `find` (elements by role or text), `page_text`, the page
