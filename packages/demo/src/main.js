...
// Auth check for protected storages - throws 401 when not authenticated
const authCheck = () => !!authAdapter.getUser()

// Unprotected storages for internal use:
// - usersStorageInternal: authAdapter needs to query users before auth (login validation)
// - booksStorageInternal: enrichment in LoansManager._enrichLoan() (loan access already validated)
const usersStorageInternal = new MockApiStorage({
  entityName: 'users',
  initialData: usersFixture
})
const booksStorageInternal = new MockApiStorage({
  entityName: 'books',
...
export { usersStorageInternal as usersStorage }
...
class LoansManager extends EntityManager {
  ...
  async _enrichLoan(data) {
    // Use internal (unprotected) storages for enrichment
    // The loan access was already auth-checked by the protected loansStorage
    if (data.book_id) {
