...
- 897c712: The notification badge flashes the moment a live update reaches the record on screen (#2679).
...
- 2c4c4ca: The notification badge keeps a trace of what happened, and shows what is in progress (#2677).
...
- 727edfb: A detail page can subscribe to its record on the stream: `sse.subscriptions` (#2664).
...
- d477b6d: Overlapping reloads no longer put a past state on screen (#2666).
...
- 72749e8: `position: sticky` works in page content again (#2534).
...
- c699ccc: A detail page keeps its active tab when the record reloads (#2435).
...
- 71e8c55: A `security.grant` judge that loads its answers can hold the first navigation: `ready()` (#2412).
...
- afaa862: `ROLE_ANONYMOUS` stays undeletable when `RolesManager` is given another `idField` (#2406).
